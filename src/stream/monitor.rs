...
use crate::platform::{PlatformConfig, PipelineOutcome};
...
async fn verify_hw_encoder(encoder: &str) -> Result<(), String> {
    ...
    fn short_reason(stderr: &str) -> String {
        ...
        if s.contains("cuda_error_no_device") || s.contains("cuinit(0) failed") || s.contains("no cuda") {
        ...
        stderr.lines().find(|l| !l.trim().is_empty()).map(|l| l.trim().to_string()).unwrap_or_else(|| "unknown error".into())
    }
...
pub async fn detect_best_hw_encoder(bitrate: &str) -> Option<(String, Vec<String>)> {
    ...
    let encoders_out = match tokio::process::Command::new("ffmpeg").args(["-hide_banner", "-encoders"]).output().await {
    ...
    let candidates: Vec<(&str, Vec<String>)> = vec![
        ("h264_nvenc", vec!["-preset".into(), "p4".into(), "-b:v".into(), bitrate.into()]),
        ("hevc_nvenc", vec!["-preset".into(), "p4".into(), "-b:v".into(), bitrate.into()]),
        ("h264_qsv", vec!["-b:v".into(), bitrate.into()]),
        ("hevc_qsv", vec!["-b:v".into(), bitrate.into()]),
        ("h264_vaapi", vec!["-vf".into(), "format=nv12,hwupload".into(), "-b:v".into(), bitrate.into()]),
        ("hevc_vaapi", vec!["-vf".into(), "format=nv12,hwupload".into(), "-b:v".into(), bitrate.into()]),
...
pub async fn probe_hw_encoders() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for line in encoders_out.lines() {
        ...
        if l.contains("nvenc") || l.contains("qsv") || l.contains("amf") || l.contains("vaapi") || l.contains("videotoolbox") || l.contains("omx") || l.contains("v4l2m2m") {
            println!("  {}", line.trim());
        }
    }
        ...
        match uploader.upload_file(file_path, config).await {
            Ok(result) => {
                if !result.urls.is_empty() {
                    upload_results.entry(uploader.name().to_string())
                        .or_default()
                        .extend(result.urls);
                    if retry_count > 0 {
                        println!("{} upload succeeded on retry attempt {}", uploader.name(), retry_count);
                    }
                } else {
            ...
            Err(e) => {
                if retry_count == 0 {
                    ...
                    eprintln!("{} upload failed on retry attempt {}: {}", uploader.name(), retry_count, e);
                }
                ...
                if is_server_error(e.status_code) && retry_count < max_retries {
                    ...
                    println!("Retrying {} upload (attempt {}/{}) after {} seconds: {}",
                             uploader.name(), retry_count + 1, max_retries, backoff.as_secs(), file_path);
                    sleep(backoff).await;
                    retry_count += 1;
                } else {
                    if retry_count >= max_retries {
                        eprintln!("Max retries ({}) exceeded for {} upload: {}", max_retries, uploader.name(), file_path);
                    }
...
impl StreamRecorder {
    async fn new(cmd: &mut tokio::process::Command) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
...
#[cfg(feature = "discord")]
async fn send_discord_webhook(webhook_url: Option<&str>, message: Message, attachment_paths: Option<Vec<String>>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
async fn send_discord_webhook(_webhook_url: Option<&str>, _message: Message, _attachment_paths: Option<Vec<String>>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // No-op when discord feature is disabled
    Ok(())
}

/// Builds ffmpeg arguments for recording with hardware acceleration when available.
fn build_ffmpeg_args(playback_url: &str, output_path: &str, hw_encoder: Option<(String, Vec<String>)>) -> Vec<String> {
        ...
        match run_pipeline(username, platform, token).await {
            Ok(PipelineOutcome::Live(vars)) => {
                ...
                if let Some(url) = playback_url {
                    let stream_title = vars.get("stream_title")
                        .map(|s| platform.clean_title(s))
                        .unwrap_or_default();
                    let user_id = vars.get("user_id")
                        .cloned()
...
async fn get_video_metadata(output_path: &str) -> Result<(f64, f64), Box<dyn std::error::Error + Send + Sync>> {
    let metadata = std::fs::metadata(output_path)?;
    let file_size_bytes = metadata.len();
    let file_size_mb = file_size_bytes as f64 / (1024.0 * 1024.0);

    let duration_minutes = match tokio::process::Command::new("ffprobe")
        .args(["-v", "quiet", "-print_format", "json", "-show_format", output_path])
        .output()
        .await
    {
        Ok(output) => {
            if let Ok(json) = serde_json::from_slice::<Value>(&output.stdout) {
                json["format"]["duration"].as_str().unwrap_or("0").parse::<f64>().unwrap_or(0.0) / 60.0
            } else {
                0.0
            }
        }
...
async fn delete_video_and_thumbnail(output_path: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(file_stem) = output_path_buf.file_stem() {
        let thumbnail_path = output_path_buf.with_file_name(format!("{}_thumb.jpg", file_stem.to_string_lossy()));
...
pub async fn build_uploaders() -> Vec<(Box<dyn Uploader>, UploaderConfig)> {
    ...
    if let Some(token) = gofile_token {
        uploaders.push((
            ...
            UploaderConfig { token: Some(token), ..Default::default() },
        ));
    } else {
    ...
    uploaders.push((Box::new(FileditchUploader::new()), UploaderConfig::default()));

    // Filester - no token required
    let filester_token = crate::utils::get_filester_token();
    uploaders.push((Box::new(FilesterUploader::new(filester_token)), UploaderConfig::default()));
...
async fn post_process_stream(stream_info: StreamInfo, output_path: String) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let webhook_url = config.get_discord_webhook_url();
    if let Err(e) = send_recording_complete_webhook(webhook_url, &stream_info, &duration_str, &size_str).await {
        eprintln!("Error sending recorded webhook: {}", e);
    }

    // Generate thumbnail
    let thumbnail_path = output_path.replace(".mp4", "_thumb.jpg");
    match create_video_thumbnail_grid(std::path::Path::new(&output_path), std::path::Path::new(&thumbnail_path), None, None).await {
    ...
    for (uploader, uploader_config) in &uploaders {
        ...
        try_upload(uploader.as_ref(), &output_path, &config, &mut upload_results, max_retries).await;
    }
    ...
    template_context.insert("username".to_string(), TemplateValue::String(stream_info.username.clone()));
    template_context.insert("user_id".to_string(), TemplateValue::String(stream_info.user_id.clone()));
    template_context.insert("output_path".to_string(), TemplateValue::String(output_path.clone()));
    template_context.insert("thumbnail_path".to_string(), TemplateValue::String(thumbnail_path.clone()));
    template_context.insert("stream_title".to_string(), TemplateValue::String(stream_info.stream_title.clone()));
    for (uploader, urls) in &upload_results {
...
async fn manage_disk_space() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("Checking disk space: output_dir = {}, free_gb = {:.2}, min_free_gb = {:.2}", output_dir, free_gb, min_free_gb);
    if free_bytes >= min_free_bytes {
        return Ok(());
    }

    println!("Free space {} GB is below minimum {} GB, cleaning up old streams...", free_bytes as f64 / 1_000_000_000.0, min_free_gb);
    ...
    for (mp4_path, _) in files {
        ...
        // Delete thumbnail
        let thumb_path = mp4_path.with_file_name(format!("{}_thumb.jpg", mp4_path.file_stem().unwrap().to_string_lossy()));
