...
impl Uploader for FilesterUploader {
	async fn upload_file(&self, file_path: &str, _config: &UploaderConfig) -> Result<UploadResult, UploadError> {
		...
		let file = tokio::fs::File::open(file_path).await.map_err(|e| UploadError {
			message: e.to_string(),
			status_code: None,
		})?;
		...
		let parsed: FilesterResponse = serde_json::from_value(raw_response.clone()).map_err(|e| UploadError {
	...
	async fn get_folder_id_by_name(&self, folder_name: &str) -> Result<Option<String>, UploadError> {
		...
		let folder_resp: FilesterFolderResponse = resp
			.json()
			.await
			.map_err(|e| UploadError {
				message: e.to_string(),
